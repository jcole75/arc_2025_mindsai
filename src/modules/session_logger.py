...
_SENSITIVE_ENV_KEYWORDS = ("KEY", "TOKEN", "SECRET", "PASSWORD", "PASS", "PRIVATE", "CREDENTIAL")
...
def _capture_safe_environment_variables(prefixes: tuple[str, ...]) -> dict[str, str]:
    captured: dict[str, str] = {}
    for key, value in os.environ.items():
        if not key.startswith(prefixes):
            continue
        if _is_sensitive_env_key(key):
            captured[key] = "<set>" if value else "<unset>"
        else:
    ...
    def _initialize_session(self):
        ...
        self.session_data["environment"] = {
            ...
            "environment_variables": {
                # Avoid writing secrets to logs (e.g., KAGGLE_KEY) while still capturing useful runtime context.
                **_capture_safe_environment_variables(("CUDA", "HF_", "KAGGLE", "TPU"))
            },
        }
